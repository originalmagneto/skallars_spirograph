    ...
    const saveSettings = async () => {
        ...
        try {
            for (const setting of settings) {
                // Upsert logic - since 'key' is PK, we can probably just upsert
                // But if using `settings` with an auto 'id', we need careful update.
                // Our schema defined `key` as PK, so upsert on `key` is easier.
                // Wait, previous schema used `key text primary key`.
                // So standard upsert works.
